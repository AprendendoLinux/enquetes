...
from sqlalchemy import or_  # <--- IMPORTANTE: Importação nova para busca (OR)
...
def admin_dashboard(
    request: Request, 
    # Novos parâmetros de busca (opcionais)
    q_users: str = None, 
    q_polls: str = None, 
    ...
    if admin.email == "admin@admin": return RedirectResponse("/admin/setup", status_code=303)

    # --- BUSCA DE USUÁRIOS ---
    users_query = db.query(models.User)
    if q_users:
        # Busca por Nome, Sobrenome OU Email (Case insensitive com ilike ou like)
    ...
    # Processamento adicional (Contagem e E-mail do criador)
    for p in polls:
    ...
    return templates.TemplateResponse("admin_dashboard.html", {
        ...
        "users": users, 
        "polls": polls,
        "q_users": q_users, # Devolvemos para o HTML preencher o campo
        "q_polls": q_polls
    })

# --- ROTA DE CONFIGURAÇÃO INICIAL (Mantida Igual) ---
@router.get("/setup", response_class=HTMLResponse)
def admin_setup_page(request: Request, db: Session = Depends(get_db)):
    admin = get_current_admin(request, db)
    if not admin: return RedirectResponse("/login", status_code=303)
    if admin.email != "admin@admin": return RedirectResponse("/admin", status_code=303)
    return templates.TemplateResponse("admin_setup.html", {"request": request, "admin": admin})

@router.post("/setup")
def admin_setup_action(request: Request, first_name: str = Form(...), last_name: str = Form(...), new_email: str = Form(...), new_password: str = Form(...), confirm_password: str = Form(...), db: Session = Depends(get_db)):
    admin = get_current_admin(request, db)
    if not admin or admin.email != "admin@admin": return RedirectResponse("/login", status_code=303)
    if new_password != confirm_password: return templates.TemplateResponse("admin_setup.html", {"request": request, "admin": admin, "error": "As senhas não coincidem."})
    if new_email != admin.email:
        exists = crud.get_user_by_email(db, new_email)
        if exists: return templates.TemplateResponse("admin_setup.html", {"request": request, "admin": admin, "error": "Este e-mail já está em uso."})
